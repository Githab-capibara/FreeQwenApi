# API Reference

This directory contains API endpoint documentation and usage examples.

## Contents

| Guide | Purpose |
|-------|---------|
| [01-api-endpoints-reference.md](01-api-endpoints-reference.md) | Complete API endpoint reference |
| [02-api-usage-examples.md](02-api-usage-examples.md) | Code examples in multiple languages |

## Base URL

```
http://localhost:3264/api
```

## Cross-References

→ [Node.js Examples](../examples/01-nodejs-examples.md)
→ [Python Examples](../examples/02-python-examples.md)
→ [Environment Variables](../reference/01-environment-variables.md)
→ [Troubleshooting](../troubleshooting/README.md)

## Template

Use [`template.md`](template.md) as a starting point for new API documents.
